class GeneratedIR():
    ...
    def assign_double(self, id, globalNames, value):
        if globalNames:
            self.buffer += "store double " + str(value) + ", double* @" + str(id) + "\n"
        else:
    ...
    def eq0(self, value1, value2, type):
        ...
        if type.count("INTEGER"):
            self.buffer += "%" + str(self.reg) + " = icmp eq i32 " + str(value1) + ", " + str(value2) + "\n"
...
def TokenIR(tree):
    ...
    # for i in range(0,70):
    #     temp = tree.index("ident")
    #     for j in range(temp,len(tree)):
    #
    #         if tree[j] == ")":
    #             varName =  tree[j+6:j+9]
    #             print(varName)
    #             j+=9
    #             break
    #         if tree[j] == "(":
    #             for k in range(j,len(tree)):
    #                 if tree[k] == ")":
    #                     value = tree[j+6:k]
    #                     print(value)
    #                     break
    ...
    ir.declare_double("max",True, 100)
    ir.assign_double("max",True,100)
    ir.declare_i32("arg",True,0)
    ir.declare_i32("ret", True, 0)
    ir.eq0(1,2,"INTEGER")
    return ir.String_generate()